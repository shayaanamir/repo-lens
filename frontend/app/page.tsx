...
export default function Home() {
  ...
  return (
    <div className="flex-1 bg-[#0B1220] text-[#ECE7D8]">
      ...
      {/* Hero                                                        */}
      {/* ---------------------------------------------------------- */}
      <section className="mx-auto grid max-w-5xl gap-12 px-6 pb-20 pt-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">        <div>
...
function HeroIllustration() {
  return (
    <svg
      ...
      width="380"
      height="220"
      className="w-full rounded-xl border border-[#1E2C40] bg-[#121C2B]"
    >
      <text x="20" y="34" className="font-[family-name:var(--font-mono)]" fontSize="13" fill="#6E7B8F">
      ...
      <rect x="0" y="0" width="380" height="3" fill="url(#rl-gradient)" className="rl-scanbar" />
      <defs>
...
function RepositoryList() {
  ...
  if (error) {
    return <p className="font-[family-name:var(--font-mono)] text-xs text-[#F2555C]">{error}</p>;
  }

  // Loading, or nothing imported yet — the illustration is a friendlier
  // hold than an empty box or a spinner in this slot.
  if (repos === null || repos.length === 0) {
    return <HeroIllustration />;
  }

  return (
    <div className="w-full rounded-xl border border-[#1E2C40] bg-[#121C2B]">
      <div className="border-b border-[#1E2C40] px-4 py-3">
        <p className="font-[family-name:var(--font-mono)] text-xs uppercase tracking-widest text-[#5FD3C4]">
          Your repositories
        </p>
      </div>
      <div className="max-h-[420px] divide-y divide-[#1E2C40] overflow-y-auto">
        {repos.map((repo) => (
          <div
            key={repo.id}
            role="button"
            ...
            className="group flex w-full cursor-pointer items-center justify-between gap-3 px-4 py-3 text-left transition-colors hover:bg-[#16202F] focus:bg-[#16202F] focus:outline-none"
          >
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="truncate font-[family-name:var(--font-display)] text-sm text-[#ECE7D8]">
                  {repo.name}
                </span>
                <StatusBadge status={repo.status} />
              </div>
              <p className="mt-0.5 truncate font-[family-name:var(--font-mono)] text-[11px] text-[#8B98AC]">
            ...
            <div className="flex shrink-0 items-center gap-1">
              <ExternalLink className="h-3.5 w-3.5 text-[#8B98AC] opacity-0 transition-opacity group-hover:opacity-100" />
              <button
                type="button"
